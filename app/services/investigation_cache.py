...
def list_cached_investigations(max_age_hours: int = 24) -> list[dict]:
    """
    Lists all cached investigation results that haven't expired.
    Returns a list of dicts with metadata about each cached investigation.
    """
    results = []
    ...
    for cache_file in CACHE_DIR.glob("*.json"):
        try:
            ...
            repo_name = data.get("plan", {}).get("repository_name", "Unknown")
...
def load_by_filename(filename: str) -> Optional[InvestigationResult]:
    """Load a cached investigation result by its filename."""
    try:
